...
class BigQueryWorker(Worker):
    def __init__(
        ...
        self.bq_table_name = self.table_name
        self.bq_table_id = f"{self.bq_project_id}.{self.bq_dataset_id}.{self.bq_table_name}"
        self.bq_schema = []
    ...
    def dump(self, **kwargs) -> None:
        ...
        table_id = f"{self.bq_table_id}"
        self.parse_column_types()

        # Configure job to load data
        job_config = bigquery.LoadJobConfig(
            source_format=bigquery.SourceFormat.CSV,
            skip_leading_rows=0,  # Skip header row in CSV
            field_delimiter=',',
    ...
    # Delete the table
        try:
            query = f"""
                TRUNCATE TABLE `{self.bq_table_id}`;
            """
            result = self.client.query(query)

        except Exception as e:
            print(f"An error occurred while deleting the table {self.bq_table_id}: {e}")
